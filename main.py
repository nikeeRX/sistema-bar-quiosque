...
async def dashboard(request: Request):
    ...
    with engine.connect() as conn:
        ...
        # --- DADOS DE ESTOQUE ---
        est_kpi = conn.execute(text("SELECT SUM(estoque) as total_itens, COUNT(*) as total_skus FROM produtos")).fetchone()
        total_itens_estoque, total_skus = int(est_kpi.total_itens or 0), int(est_kpi.total_skus or 0)
        
        est_cat = conn.execute(text("SELECT categoria, SUM(estoque) as qtd FROM produtos GROUP BY categoria")).fetchall()
        labels_est_cat, data_est_cat = [r.categoria for r in est_cat], [int(r.qtd or 0) for r in est_cat]
        
        est_critico = conn.execute(text("SELECT nome, estoque FROM produtos WHERE estoque <= 15 ORDER BY estoque ASC LIMIT 5")).fetchall()
        labels_critico, data_critico = [r.nome for r in est_critico], [r.estoque for r in est_critico]

        cruzamento = conn.execute(text("SELECT p.nome, p.estoque, COUNT(v.id) as vendidos FROM produtos p LEFT JOIN vendas_itens v ON p.nome = v.item_nome AND v.status = 'FECHADA' GROUP BY p.nome, p.estoque ORDER BY vendidos DESC LIMIT 5")).fetchall()
    ...
    dash_css = """<style>.grid-dash { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px; width: 100%; max-width: 1100px; margin-bottom: 30px; } .card-kpi { background: white; padding: 20px; border-radius: 10px; color: #333; box-shadow: 0 4px 6px rgba(0,0,0,0.1); border-left: 5px solid #d31a21; } .card-kpi.estoque { border-left-color: #e67e22; } .card-kpi h3 { margin: 0; font-size: 14px; color: #666; text-transform: uppercase; } .card-kpi p { margin: 10px 0 0; font-size: 24px; font-weight: bold; color: #0a3a7a; } .chart-container { background: white; padding: 20px; border-radius: 10px; margin-bottom: 20px; width: 100%; max-width: 530px; display: inline-block; vertical-align: top; } .aba-btn { background: #062b5e; color: white; border: none; padding: 15px 30px; font-size: 16px; font-weight: bold; border-radius: 8px; cursor: pointer; margin-right: 10px; transition: 0.3s; } .aba-btn:hover { background: #d31a21; }</style><script src="https://cdn.jsdelivr.net/npm/chart.js"></script>"""

    return f"""<html><head>{CSS}{dash_css}</head><body><div class='main-area' style='padding: 30px; overflow-y: auto;'><h1 style='color:white; margin-bottom: 10px;'>📊 Painel de Gestão</h1><div style='margin-bottom: 30px;'><button id='btn-fin' class='aba-btn' style='background:#17a2b8' onclick="showTab('fin')">💰 VISÃO FINANCEIRA</button><button id='btn-est' class='aba-btn' style='opacity:0.6; background:#e67e22' onclick="showTab('est')">📦 VISÃO ESTOQUE</button></div><div id='tab-fin'><div class='grid-dash'><div class='card-kpi'><h3>Faturamento Bruto</h3><p>R$ {faturamento_total:.2f}</p></div><div class='card-kpi'><h3>Ticket Médio</h3><p>R$ {ticket_medio:.2f}</p></div><div class='card-kpi'><h3>Comandas Fechadas</h3><p>{total_comandas}</p></div></div><div style='width: 100%; max-width: 1100px;'><div class='chart-container'><h3 style='color:#333'>💰 Meios de Pagamento</h3><canvas id="chartPag"></canvas></div><div class='chart-container'><h3 style='color:#333'>🔝 Top 5 Produtos Vendidos</h3><canvas id="chartProd"></canvas></div></div><div class='card-center' style='max-width: 1100px; text-align: left; margin-top:20px;'><h3 style='color:black; margin-top:0'>👨‍🍳 Performance por Garçom</h3><table><tr style='color:black; border-bottom:2px solid #ccc'><th>Garçom</th><th>Total Vendido (R$)</th></tr>{"".join([f"<tr><td style='color:black'>{g.garcom or 'Não Identificado'}</td><td style='color:black; font-weight:bold'>R$ {float(g.total):.2f}</td></tr>" for g in garcons])}</table></div></div><div id='tab-est' style='display:none;'><div class='grid-dash'><div class='card-kpi estoque'><h3>Volume Total no Estoque</h3><p>{total_itens_estoque} Itens</p></div><div class='card-kpi estoque'><h3>Produtos Cadastrados (SKUs)</h3><p>{total_skus}</p></div></div><div style='width: 100%; max-width: 1100px;'><div class='chart-container'><h3 style='color:#333'>📦 Volume por Categoria</h3><canvas id="chartCatEstoque"></canvas></div><div class='chart-container'><h3 style='color:#333'>⚠️ Alerta de Estoque Crítico</h3><canvas id="chartCritico"></canvas></div><div class='chart-container' style='max-width: 100%; display:block;'><h3 style='color:#333'>🔄 Cruzamento: Vendidos vs Estoque Atual (Top 5)</h3><canvas id="chartCruzamento"></canvas></div></div></div><br><a href='/central' class='btn-acao' style='width: 200px; margin:auto'>Voltar ao Início</a></div><script>function showTab(tab) {{ document.getElementById('tab-fin').style.display = tab === 'fin' ? 'block' : 'none'; document.getElementById('tab-est').style.display = tab === 'est' ? 'block' : 'none'; document.getElementById('btn-fin').style.opacity = tab === 'fin' ? '1' : '0.6'; document.getElementById('btn-est').style.opacity = tab === 'est' ? '1' : '0.6'; }} new Chart(document.getElementById('chartPag'), {{ type: 'doughnut', data: {{ labels: {json.dumps(labels_pag)}, datasets: [{{ data: {json.dumps(data_pag)}, backgroundColor: ['#0a3a7a', '#d31a21', '#ffc107', '#28a745'] }}] }} }}); new Chart(document.getElementById('chartProd'), {{ type: 'bar', data: {{ labels: {json.dumps(labels_prod)}, datasets: [{{ label: 'Qtd Vendida', data: {json.dumps(data_prod)}, backgroundColor: '#28a745' }}] }} }}); new Chart(document.getElementById('chartCatEstoque'), {{ type: 'doughnut', data: {{ labels: {json.dumps(labels_est_cat)}, datasets: [{{ data: {json.dumps(data_est_cat)}, backgroundColor: ['#e67e22', '#3498db', '#9b59b6', '#34495e', '#f1c40f'] }}] }} }}); new Chart(document.getElementById('chartCritico'), {{ type: 'bar', data: {{ labels: {json.dumps(labels_critico)}, datasets: [{{ label: 'Qtd Restante', data: {json.dumps(data_critico)}, backgroundColor: '#d31a21' }}] }}, options: {{ indexAxis: 'y' }} }}); new Chart(document.getElementById('chartCruzamento'), {{ type: 'bar', data: {{ labels: {json.dumps(labels_cruz)}, datasets: [{{ label: 'Total Vendido', data: {json.dumps(data_cruz_vendidos)}, backgroundColor: '#28a745' }}, {{ label: 'Estoque Atual', data: {json.dumps(data_cruz_estoque)}, backgroundColor: '#0a3a7a' }}] }} }});</script></body></html>"""
