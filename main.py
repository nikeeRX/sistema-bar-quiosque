...
# --- CRIAÇÃO DAS TABELAS BÁSICAS E HISTÓRICO DE ESTOQUE ---
...
async def central(request: Request):
    ...
    popup = ""
    if user == "admin":
        botoes += "<a href='/dashboard' class='btn-acao' style='background:#17a2b8'>📊 DEMONSTRATIVO DE GESTÃO</a><a href='/estoque' class='btn-acao' style='background:#e67e22'>📦 GESTÃO DE ESTOQUE</a><a href='#' class='btn-acao' style='background:#062b5e' onclick='abrirPopup()'>⚙️ CONECTAR IMPRESSORA</a>"
...
async def tela_estoque(request: Request):
    ...
    with engine.connect() as conn:
        # AGORA PUXA A DATA DA ÚLTIMA COMPRA DA TABELA DE HISTÓRICO
        prods_db = conn.execute(text("""
            SELECT p.id, p.nome, p.categoria, p.preco, p.estoque, MAX(h.data_entrada) as ultima_compra 
            FROM produtos p 
            LEFT JOIN historico_estoque h ON p.nome = h.produto_nome 
