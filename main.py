...
def emitir_nfe_api(cpf, itens, total, pag):
    ...
    if not conf or not conf.focus_token: return "⚠️ Erro: API NFe não configurada no painel."
...
async def dashboard(request: Request, inicio: str = "", fim: str = "", cat: str = "", prod: str = "", garcom_filtro: str = ""):
    ...
    with engine.connect() as conn:
        ...
        total_entradas_db = conn.execute(text(f"SELECT SUM(qtd_adicionada) as total_entrou FROM historico_estoque WHERE {where_hist}")).fetchone()
...
async def novo_produto(request: Request):
    ...
    try:
        with engine.begin() as conn: 
            conn.execute(text("INSERT INTO produtos (nome, category, preco, estoque) VALUES (:n, :c, :p, :q) ON CONFLICT (nome) DO NOTHING"), {"n": f.get("focus_token"), "c": f.get("cat"), "p": float(f.get("preco").replace(",", ".")), "q": int(f.get("qtd"))})
...
async def editar_produto(request: Request):
    f = await request.form()
    n_orig, n_novo, cat, p = f.get("nome_orig"), f.get("focus_token"), f.get("cat"), float(f.get("preco").replace(",", "."))
    try:
        with engine.begin() as conn:
