...
IMG_LOGO = """<div style='display:flex; justify-content:center; margin-bottom:20px;'><img src='https://upload.wikimedia.org/wikipedia/commons/thumb/1/18/Brahma_Logo.svg/512px-Brahma_Logo.svg.png' class='logo-central'></div>"""
IMG_LOGO_PEQ = """<div style='display:flex; justify-content:center; margin-bottom:15px;'><img src='https://upload.wikimedia.org/wikipedia/commons/thumb/1/18/Brahma_Logo.svg/512px-Brahma_Logo.svg.png' class='logo-peq'></div>"""

@app.get("/sw.js")
async def get_sw(): return Response(content="self.addEventListener('install', e => { self.skipWaiting(); }); self.addEventListener('fetch', e => {});", media_type="application/javascript")
@app.get("/manifest.json")
async def get_manifest(): return {"name": "Quiosque Brahma", "short_name": "BrahmaApp", "start_url": "/", "display": "standalone", "background_color": "#0a3a7a", "theme_color": "#d31a21", "icons": [{"src": "https://upload.wikimedia.org/wikipedia/commons/thumb/1/18/Brahma_Logo.svg/512px-Brahma_Logo.svg.png", "sizes": "512x512", "type": "image/png"}]}
...
# ==========================================
# CARDÁPIO DIGITAL & QR CODE (A MÁGICA)
# ==========================================
...
async def gerar_qr(request: Request):
    ...
    return f"""<html><head>{CSS}</head><body><div class='container-center'><div class='card-center'>{IMG_LOGO_PEQ}<h2 style='color:#d31a21;'>QR Code do Cardápio</h2><p style='color:#333;'>Imprima esta imagem e coloque nas mesas. O cardápio atualiza automaticamente conforme as vendas!</p><div style='background:white; padding:20px; display:inline-block; border-radius:15px; border:2px dashed #ccc; margin:20px 0;'><img src='{qr_url}' style='width:250px; height:250px;'></div><br><a href='{link_cardapio}' target='_blank' style='color:#062b5e; font-weight:bold; font-size:18px; text-decoration:underline;'>🔗 Acessar Link do Cardápio</a><br><br><br><a href='/central' class='btn-acao' style='background:#333;'>Voltar ao Painel</a></div></div></body></html>"""


# ==========================================
# MÓDULO DE CAIXA COMPLETO
# ==========================================
...
# ==========================================
# DASHBOARD GERENCIAL COMPLETÃO (GRÁFICOS)
# ==========================================
...
async def tela_comissoes(request: Request, garcom_filtro: str = ""):
    ...
    with engine.connect() as conn:
        ...
        for r in res_pend:
            ...
            acoes = f"<form action='/pagar_comissao' method='post' style='margin:0;' onsubmit='return confirm(\"Pagar comissão de {r.garcom}?\");'><input type='hidden' name='data_venda' value='{r.data}'><input type='hidden' name='garcom' value='{r.garcom}'><button class='btn-acao' style='background:#28a745; margin:0; padding:8px; width:auto; font-size:12px;'>✔️ PAGO</button></form>"
...
# ==========================================
# LANÇAMENTO DE VENDAS E FECHAMENTO DE CONTA COM NFC-e
# ==========================================
...
# ==========================================
# UTILITÁRIOS (LOGOUT / API IMPRESSÃO)
# ==========================================
@app.get("/logout")
